...
export default function MobileHeader({ open, onToggle }) {
  ...
  return (
    <>
      <header className="relative z-50 flex h-16 items-center justify-between border-b border-neutral-200 bg-white px-4 dark:border-neutral-800 dark:bg-neutral-900 md:hidden">
      ...
      <div id="mobile-navigation-drawer" className={`fixed inset-x-0 bottom-0 top-16 z-40 bg-black/30 transition-opacity duration-200 ease-out md:hidden ${open ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`} aria-hidden={!open} inert={!open ? '' : undefined} onClick={onToggle}>
